  ...
  generateDoc() {
    ...
    if (editor) {
      ...
      if(curPos.row == 0 && (thisLine.match(funcRegex)==null)){
        ...
        if (lineaboveFunction != null && lineaboveFunction.match(/^\s*\/\/ #doco-end/)) {
          let row = curPos.row - 1
          let line = null
          while (line = buffer.lineForRow(row)) {
            if (line.match(/^\s*\/\/ #doco-start/)) {

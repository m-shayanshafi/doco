...
export default {
  ...
  config: {
    ...
    "classpath": {
      ...
      "type": "array",
      "default": ["../guava/guava/target/classes", "/home/mshayan/.m2/repository/org/checkerframework/checker-compat-qual/2.0.0/checker-compat-qual-2.0.0.jar"]
    },
    "tests_dir": {
      "description": "Test Directory",
      "type": "string",
      "default": "/opt/guava/guava-tests/test"
    }
  },
  ...
  generateDoc() {
    ...
    docoPath = packageDirectory + '/doco' + '/src'  + '/bin' + '/doco'
    ...
    if(docGenerated){
      ...
      console.log(configs)
      commandtoRun = "./doco '"+  JSON.stringify(configs) + "' '" + packageString + "' '" + classString + "' '" + funcString + "'"

      runCommand(commandtoRun).then(function({stdout}){
        console.log(stdout)
        insertPos = [funcAt, 0]
        buffer.insert(insertPos, "\n\n\n")
        buffer.insert(insertPos, "// Precondition: x > 0")
...
async function runCommand(cmd) {
  ...
  var std = await new Promise(function (resolve, reject) {
    exec(cmd, {cwd: docoPath},(err, stdout, stderr) => {
      if (err) {
        reject(err);
      } else {
